...
const video = {
    converter(input, output, bitRate) {
        return new Promise((resolve, reject) => {
            let cmd = ['-i', input, output, '-loglevel', 'error', '-y'];
            if (bitRate) {
                cmd = ['-i', input, '-b:a', bitRate.ba + 'k', '-b:v', bitRate.bv + 'k', output, '-loglevel', 'error', '-y'];
            }
    ...
    desktopRecord(videoSource, audioSource, videoSize = "1024x768", offset = {
        x: 0,
        y: 0
    }, range = 60, output) {
        return new Promise((resolve, reject) => {
            let commonCmdArr = ['-rtbufsize', '100M'];
            let videoCmdArr;
            if (videoSource == 'gdigrab') {
                videoCmdArr = ['-f', 'gdigrab', '-framerate', range, '-video_size', videoSize, '-offset_x', offset.x, '-offset_y', offset.y, '-r', range, '-i', 'desktop'];
            } else {
                videoCmdArr = ['-f', 'dshow', '-i', 'video=' + videoSource];
            }

            let endCmdArr = [output];
            let audioCmdArr = [];
            if (audioSource) {
                audioCmdArr = ['-f', 'dshow', '-i', 'audio=' + audioSource];
            }
            let cmd = commonCmdArr.concat(audioCmdArr).concat(videoCmdArr).concat(endCmdArr);
            console.log(cmd);
            const child = child_process.execFile(ffmpeg, cmd, (error, stdout, stderr) => {
                if (error || stderr) {
                    console.log(error, stderr);
