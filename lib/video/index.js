...
const video = {
    ...
    autoVideoshot(input, timespan, output) {
        return new Promise((resolve, reject) => {
            const child = child_process.execFile(ffmpeg, ['-i', input, '-vf', 'fps=1', output, '-loglevel', 'error', '-y'], (error, stdout, stderr) => {
                if (error || stderr) {
                    console.log(error, stderr);
                    reject(error, stderr);
                } else {
    ...
    desktopScreenShot(output) {
        return new Promise((resolve, reject) => {
            const child = child_process.execFile(ffmpeg, ['-f', 'gdigrab', '-i', 'desktop', '-vframes', '1', output, '-loglevel', 'error', '-y'], (error, stdout, stderr) => {
                if (error || stderr) {
                    console.log(error, stderr);
                    reject(error, stderr);
                } else {
    ...
    desktopScreenVideo(videoSize = "1024x768", offset = {
        x: 0,
        y: 0
    }, range = 30, output) {
        return new Promise((resolve, reject) => {
            const child = child_process.execFile(ffmpeg, ['-f', 'gdigrab', '-video_size', videoSize, '-offset_x', offset.x, '-offset_y', offset.y, '-r', range, '-i', 'desktop', output, '-loglevel', 'error', '-y'], (error, stdout, stderr) => {
                if (error || stderr) {
                    console.log(error, stderr);
                    reject(error, stderr);
                } else {
