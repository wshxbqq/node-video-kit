...
var app = new Vue({
    ...
    mounted: function () {
        let _this = this;
        util.getDevicesList().then(devicesList => {
            devicesList.audioDevices.map(ad => {
                app.$data.audioList.push(ad);
            });
            devicesList.videoDevices.map(ad => {
                app.$data.videoList.push(ad);
            });
            app.$data.videoSource = 0;
        })
    },
    computed: {
        videoName() {
        ...
        withSound(val, oldVal) {
            if (val) {
                if (!app.$data.audioSource) {
                    ...
                    app.$data.audioSource = 0;
                }
            } else {
                app.$data.audioSource = "";
            }
        }
    },
    methods: {
        ...
        goRecord(e) {
            ...
            videokit.video.desktopRecord(app.$data.videoSource, app.$data.audioSource, 60, outPutFullFileName).then(child => {
                ffmpegHandle = child;
            }).catch(e => {
                console.log(e);
                app.$data.processing = false;
